...
require("../../../assets/css/certification-section.css");
class CertificationSection extends react_1.Component {
    constructor(props) {
        super(props);
    ...
    renderItem(item, index) {
        return (<flow_item_component_1.FlowItem key={item.key} itemKey={item.key} index={index} scrollDirection="left" isVisible={true} refObj={this.itemRefs.get(item.key)} icon={<span>{item.icon}</span>}>
        {this.renderContent(item)}
      </flow_item_component_1.FlowItem>);
    }
    render() {
        const { data } = this.props;
        return (<card_component_1.default id="certification-section" title="Certifications">
...
exports.default = CertificationSection;
