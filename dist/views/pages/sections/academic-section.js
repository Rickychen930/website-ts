...
require("../../../assets/css/academic-section.css");
...
class AcademicSection extends react_1.Component {
    constructor(props) {
        super(props);
    ...
    renderItem(item, index) {
        ...
        return (<flow_item_component_1.FlowItem key={item.key} itemKey={item.key} index={index} scrollDirection={scrollDirection} isVisible={visibleItems.has(item.key)} refObj={this.itemRefs.get(item.key)} icon={<span>{item.icon}</span>}>
        {this.renderContent(item)}
      </flow_item_component_1.FlowItem>);
    }
    render() {
        const { data } = this.props;
        return (<card_component_1.default id="academic-section" title="Academic Background">
...
exports.default = AcademicSection;
